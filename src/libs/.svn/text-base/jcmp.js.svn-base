...
//调用原生返回方法
jcmp.goBack=function(callback){ 
    jcmp.invokeClientMethod("goBack");
};

//关闭webview窗口
jcmp.finishCurUI=function(callback){ 
    jcmp.invokeClientMethod("finishCurUI");
};

//获取用户名
jcmp.getUserAllInfo=function(callback){ 
	jcmp.invokeClientMethod("getUserAllInfo", "", callback);
};

//获取当前H5Container最原始加载的url地址
jcmp.getCurrentOriginalUrl=function(callback){ 
	jcmp.invokeClientMethod("getCurrentOriginalUrl", "", callback);
};

//获取当前H5Container当前显示的url地址
jcmp.getCurrentLoadedUrl=function(callback){ 
	jcmp.invokeClientMethod("getCurrentLoadedUrl", "", callback);
};

//获取网络状态  "wifi"---无线网,"mobile"---移动,""----无网络
jcmp.getNetworkState=function(callback){ 
	return jcmp.invokeClientMethod("getNetworkState", "", callback);
};
...
if(jcmpUrl && jcmpUrl.length > 0){
	jcmpUrl = jcmpUrl.slice(0, jcmpUrl.indexOf("#"))
...
jcmp.isAndroid = (/android/gi).test(navigator.userAgent);
jcmp.isIOS = (/iphone|ipad/gi).test(navigator.userAgent);
console.log(navigator,jcmp.isAndroid,jcmp.isIOS)
